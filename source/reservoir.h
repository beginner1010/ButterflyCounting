...
public:
	...
	void add_edge_to_reservoir(const int &A, const int &B);
	bool remove_edge_from_reservoir(const int& A, const int& B);
	...
	inline std::unordered_map< int, std::unordered_set<int> >& get_graph() {
		return this->adj;
	}

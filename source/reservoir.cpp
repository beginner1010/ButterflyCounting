...
void reservoir::add_edge_to_reservoir(const int& A, const int& B) {
...
bool reservoir::remove_edge_from_reservoir(const int& A, const int& B) {
	long long edge_hash = this->to_hash(A, B);
	if (edge_to_index.find(edge_hash) != edge_to_index.end()) {

		this->adj[A].erase(B);
		if(this->adj[A].empty()) this->adj.erase(A);

		this->adj[B].erase(A);
		if (this->adj[B].empty()) this->adj.erase(B);
...
void reservoir::replace_in_reservoir(const int& A, const int& B) {
	int ran_idx = (int)sampler::generate_random_int(0, edge_pool.size() - 1);
...
void reservoir::replace_in_reservoir_with_fixed_size(const int& A, const int& B) {
	int ran_idx = (int)sampler::generate_random_int(0, this->max_size_of_reservoir - 1);

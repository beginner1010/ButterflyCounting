...
namespace exact {
	namespace global {
		...
		long long Kai_BFC_in_reservoir(reservoir& R) {
			long long ans = 0;
			std::unordered_map< int, std::unordered_set<int> >& g = R.get_graph();
			for(auto& p: g) {
				auto& start = p.first;
				std::unordered_map<int,int> counter;
				std::unordered_set<int>& neighbors_of_start = R.get_adj(start);
				int degree_start = (int)neighbors_of_start.size();
				for (auto& middle: neighbors_of_start) {
					std::unordered_set<int>& neighbors_of_middle = R.get_adj(middle);
					int degree_middle = (int)neighbors_of_middle.size();
					if (degree_start > degree_middle || (degree_start == degree_middle && start > middle)) {
						for (auto& end: neighbors_of_middle) {
							int degree_end = (int)R.get_adj(end).size();
							if (degree_start > degree_end || (degree_start == degree_end && start > end)) {
								ans += counter[end];
								counter[end] ++;
							}
	...
	namespace edge {
		long long ebfc(reservoir& R, const int& vertex_a, const int& vertex_b) {
			auto& adj_A = R.get_adj(vertex_a);
			auto& adj_B = R.get_adj(vertex_b);
			std::unordered_map <int, long long> counter;
			for (auto& neighbor_a: adj_A) {
				if (neighbor_a == vertex_b) continue;
				auto& two_hop_adj = R.get_adj(neighbor_a, true);
			...
			for (auto& neighbor_b: adj_B) {
				if (neighbor_b != vertex_a && counter.find(neighbor_b) != counter.end()) {

...
int Graph::get_vertex_index(const long long& vertex) {
	if (this->vertex_index.find(vertex) == this->vertex_index.end()) {
		...
		this->vertex_index[vertex] = this->n++;
		if (settings::is_static == true) {
			if ((vertex & 1) == 0)
				this->vertices_left.push_back(this->vertex_index[vertex]), this->n_left++;
			else
				this->vertices_right.push_back(this->vertex_index[vertex]), this->n_right++;
	...
	while (std::fgets(char_line, 1024, IO::fin)) {
		...
		if (((int)vec_str.size()) >= 2) {
			...
			if (is_all_num) {
				...
				int A = this->get_vertex_index(vertex_left * 10LL);
				int B = this->get_vertex_index(vertex_right * 10LL + 1);

				if (settings::is_static) {
					if (seen_edges.find(this->encode_edge(A, B)) != seen_edges.end())
						continue;
					seen_edges.insert(this->encode_edge(A, B));
					...
					/*	
						Here, we are dealing with a graph stream. We load the graph stream in vectors for sake of experiments. 
						Note that algorithms do not need to store the graph.
					*/
...
void Graph::clear() {
	this->edges.clear();
	this->edge_stream_ins_only.clear();
	this->edge_stream_fully_dynamic.clear();
	this->vertices.clear();
